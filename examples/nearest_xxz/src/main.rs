...
fn hamiltonian_with(l : usize, m : usize, k : usize) -> Result<Array1<f64>, Error>{
    ...
    return hamiltonian.eigh(UPLO::Lower)
        .map(|v| v.0)
        .map_err(|e| Error::make_error_msg(format_args!("{}", e).to_string()));
}


fn hamiltonian_with_next(l : usize, m : usize, k : usize) -> Result<Array1<f64>, Error>{
    let basis_gen = BasisNK::new(EigenNumMomentum::new(m, k), l);
    let (basis, indices) = basis_gen.build()?;
