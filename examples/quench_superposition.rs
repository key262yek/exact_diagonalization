...
fn main() -> (){
    ...
    let filepath = format_args!("examples/output/ed_sppn_p_Index_{}_SysSize_{}_M_{}_K_{}_Delta_{:0e}_Lambda_{:0e}_Ensem_{}_Seed_{}_Thr_{:0e}.dat", interaction_info, l, m, k, delta, lambda, num_en, org_seed, threshold).to_string();
    ...
    for _ in 0..num_en{
        ...
        result = result + total.map(|x| if x.re > threshold {p} else {0.0});
